...
export default async function DynamicServicePage({ params }: DynamicServiceProps) {
  ...
  return (
    <div className={`min-h-screen bg-white ${dmSans.className}`}>
      {/* Hero Section */}
      <section id="hero-section" className="relative overflow-hidden bg-[#120E07] text-white pt-24 pb-16 sm:pt-60 sm:pb-20 md:pt-44 md:pb-32 px-6 sm:px-12 md:px-16 min-h-[560px] md:min-h-[560px]">
        <div
        ...
        <div className="max-w-[1440px] mx-auto w-full relative z-20">
          <nav
            className="reveal flex items-center gap-2 text-xs sm:text-sm font-semibold tracking-widest text-[#C2943A] mb-6 uppercase"
          ...
          <h1 className="rv-up text-2xl text-[#C2943A] md:text-3xl font-bold mb-8 mt-12">
            {service.title}
          </h1>
          <p className="rv-up text-zinc-200 text-lg max-w-2xl" style={{ transitionDelay: "150ms" }}>
            {service.description}
          </p>
        </div>
      </section>
