      ...
      {isOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-4">
          ...
          <motion.button
            ...
            onClick={closeModal}
            className="fixed top-4 right-4 md:top-8 md:right-8 z-[10000] p-2 text-neutral-400 hover:text-white hover:bg-neutral-800/40 rounded-full transition-colors cursor-pointer"
            aria-label="Close modal"
          >
            <X size={24} />
          </motion.button>

          {/* Modal Container: Enabled mobile-only scrolling */}
          <motion.div
            ref={modalRef}
            layout
            ...
            className={`relative w-full md:max-w-none bg-[#131210] border border-[#2A2925] rounded-3xl shadow-[0_24px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col md:flex-row max-h-[88vh] md:max-h-[90vh] overflow-y-auto md:overflow-visible ${view === "calendar"
                ? selectedDate
                  ...
                  <AnimatePresence mode="wait">
                    {view === "form" ? (
                      ...
                      <motion.div
                        ...
                        exit={{ opacity: 0, x: 20, filter: "blur(4px)" }}
                        transition={{ duration: 0.3, ease: "easeOut" }}
                        className="flex flex-col md:flex-row h-full min-h-0 w-full overflow-y-auto md:overflow-visible pb-6 md:pb-0"
                      >
                        <LayoutGroup id="schedule-layout">
                          {/* Calendar view container (allows scrolling on mobile) */}
                          <motion.div
                            layout
                            transition={{ type: "spring", bounce: 0, duration: 0.5 }}
                            className={`flex justify-center flex-1 min-w-0 overflow-y-auto md:overflow-hidden ${mobileShowTimes ? "hidden md:flex" : "flex"}`}
                          >
                            <ScheduleCalender
